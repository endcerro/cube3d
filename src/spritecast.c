...
/*   Updated: 2020/02/28 05:26:37 by edal--ce         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_draw_s_e(t_contr *contr, t_sprite_rend *r, int i)
{
	...
	inv_det = 1.0 / (contr->plane.x * contr->dir.y - contr->dir.x *
		contr->plane.y);
	r->sprite.x = contr->sprites[r->spr_ord[i]].x - contr->pos.x;
	r->sprite.y = contr->sprites[r->spr_ord[i]].y - contr->pos.y;
	r->transform.x = inv_det * (contr->dir.y * r->sprite.x - contr->dir.x *
		r->sprite.y);
...
void	draw_stripe(t_contr *contr, t_sprite_rend *r, int stripe, int i)
{
	...
	y = r->draw_start.y - 1;
	r->tex_p.x = (int)(256 * (stripe - (-r->sprite_d.x / 2 + r->spr_screen_x))
